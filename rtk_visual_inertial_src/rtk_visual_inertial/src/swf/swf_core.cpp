...
void SWFOptimization::AddAllResidual(int mode, std::set<double*>MargePoint, MarginalizationInfo* marginalization_info,
    ...
    if ((mode == NormalMode || mode == NormalMode2 || mode == MargeIncludeMode2) && (usegnss || useimu)) {
...
#define B10_ IMUFactor* factor = new IMUFactor(pre_integrations[index2]);
                ...
                if (IMUGNSSmeasurement) {
                    ...
                    param.push_back(para_pose[index1]);
                    param.push_back(para_speed_bias[index1]);
                    param.push_back(para_pose[index2]);
                    param.push_back(para_speed_bias[index2]);
...
#define B2_ MarginalizationFactor *factor = new MarginalizationFactor(rovers[ir]->marg_info_gnss);
    ...
    if (usefeature && USE_IMAGE) {
        ...
        for (auto& it_per_id : f_manager.feature) {
            if (it_per_id.feature_per_frame.size() < FEATURE_CONTINUE || it_per_id.start_frame >= FEATURE_WINDOW_SIZE - 2)continue;
            int imu_j = it_per_id.start_frame ;
#if USE_INVERSE_DEPTH
            Vector3d pts_i = it_per_id.feature_per_frame[0].point;
#endif
            for (auto& it_per_frame : it_per_id.feature_per_frame) {
                if (imu_j == FEATURE_WINDOW_SIZE)continue;
#if USE_INVERSE_DEPTH
...
#define A132NoEx para_pose[i2f[it_per_id.start_frame]], para_pose[i2f[imu_j]], &it_per_id.idepth_
#define B132NoEx ProjectionTwoFrameOneCamFactorNoEx *factor = new ProjectionTwoFrameOneCamFactorNoEx(pts_i, pts_j,tic[0],Eigen::Quaterniond(ric[0]));
                        ADDRESIDUAL (A132NoEx, B132NoEx, false, std::vector<int> {})
                    }
                }
...
#define A133NoEx para_pose[i2f[it_per_id.start_frame]], para_pose[i2f[imu_j]], &it_per_id.idepth_
#define B133NoEx ProjectionTwoFrameTwoCamFactorNoEx *factor = new ProjectionTwoFrameTwoCamFactorNoEx(pts_i, pts_j_right,tic[0],Eigen::Quaterniond(ric[0]),tic[1],Eigen::Quaterniond(ric[1]));
                            ADDRESIDUAL (A133NoEx, B133NoEx, false, std::vector<int> {})
                        }
...
#define A134NoEx  &it_per_id.idepth_
#define B134NoEx ProjectionOneFrameTwoCamFactorNoEx *factor= new ProjectionOneFrameTwoCamFactorNoEx(pts_i, pts_j_right,tic[0],Eigen::Quaterniond(ric[0]),tic[1],Eigen::Quaterniond(ric[1]));
                            ADDRESIDUAL (A134NoEx, B134NoEx, false, std::vector<int> {})
                        }
...
#define B14 projection_factor *factor = new projection_factor(it_per_frame.pointRight);
                    ...
                    ceres::LossFunction* loss_function = new ceres::CauchyLoss(1.0);
#define A13NoEx para_pose[i2f[imu_j]],  it_per_id.ptsInWorld.data()
#define B13NoEx projection_factorNoEx1 *factor = new projection_factorNoEx1(it_per_frame.point);
                    ADDRESIDUAL (A13NoEx, B13NoEx, false, std::vector<int> {})

                    if (USE_STEREO && it_per_frame.is_stereo) {
                        ceres::LossFunction* loss_function = new ceres::CauchyLoss(1.0);
#define A14NoEx para_pose[i2f[imu_j]],   it_per_id.ptsInWorld.data()
#define B14NoEx projection_factorNoEx2 *factor = new projection_factorNoEx2(it_per_frame.pointRight);
                        ADDRESIDUAL (A14NoEx, B14NoEx, false, std::vector<int> {})
...
#define B2 MarginalizationFactor *factor = new MarginalizationFactor(last_marg_info);
    ...
    if (mode == MargeExcludeMode) {
        ...
        std::cout << "solving time55:" << t_marg.toc() << std::endl;
        ...
        if (ceres::internal::parameter_head.size() == 0) {
            ...
            MyOrdering(problem, options);
            ceres::Solve(options, &problem, &summary);
            std::cout << "solving time66:" << t_marg.toc() << std::endl;
            UpdateSchur(problem);
            ceres::internal::is_optimize = true;
...
void SWFOptimization::MiddleMargGnssFrame() {
    for (int IMUGNSSindex = 0; IMUGNSSindex < FEATURE_WINDOW_SIZE + 2; IMUGNSSindex++) {
        ...
        if (imu_gnss_factor[IMUGNSSindex]->gnss_speed_bias.size() <= GNSS_MAX_NUM_ONE_FACTOR)continue;
        ...
        int marg_num = gnss_num - GNSS_MAX_NUM_ONE_FACTOR;
        // int marg_index0=GNSS_MAX_NUM_ONE_FACTOR/2;
        std::set<int>margeindex;
        for (int i = 0; i < marg_num; i++) {
            int index = GNSS_MAX_NUM_ONE_FACTOR / 2 + i + index1;
            ASSERT(index < index2);
        ...
        ASSERT(a == 2 && b == 2 && c == 0);
        pre_integrations[GNSS_MAX_NUM_ONE_FACTOR / 2 + index1]->sum_dt = IMUINTEGRATIONTIMEMAX + 100000;
        int index = GNSS_MAX_NUM_ONE_FACTOR / 2 + index1;
        for (int i = 0; i < marg_num; i++) {
        ...
        PhaseBiasSaveAndReset();
        rovers[f2g[GNSS_MAX_NUM_ONE_FACTOR / 2 + index1]]->marg_info_gnss->ResetLinearizationPoint(rovers[f2g[GNSS_MAX_NUM_ONE_FACTOR / 2 + index1]]->marg_info_gnss->keep_block_addr);
        rovers[f2g[GNSS_MAX_NUM_ONE_FACTOR / 2 + index1 - 1]]->marg_info_gnss->ResetLinearizationPoint(rovers[f2g[GNSS_MAX_NUM_ONE_FACTOR / 2 + index1 - 1]]->marg_info_gnss->keep_block_addr);
        PhaseBiasRestore();

        ASSERT(pre_integrations[GNSS_MAX_NUM_ONE_FACTOR / 2 + index1]->sum_dt > IMUINTEGRATIONTIMEMAX + 100000);
