...
void SWFOptimization::InputImage(double t, const cv::Mat& _img, const cv::Mat& _img1) {
    ...
    if (USE_IMAGE) {
        ...
        std::cout<<"feature extraction process time:"<<t_process.toc()<<std::endl;
        mutex_feature.lock();
        feature_buf.push(make_pair(t, featureFrame));
        Save();
        mutex_feature.unlock();
    }
    if (!USE_MULTI_THREAD)MeasurementProcess();
...
void SWFOptimization::AddFeature2Problem() {
    ...
    for (auto& it_per_id : f_manager.feature) {
        ...
        for (auto& it_per_frame : it_per_id.feature_per_frame) {
            ...
            if (!it_per_frame.is_in_optimize && imu_j != FEATURE_WINDOW_SIZE) {
                ...
                if (it_per_id.start_frame != imu_j) {
                    ...
                    if (ESTIMATE_EXTRINSIC) {
                        ...
                        ProjectionTwoFrameOneCamFactorNoEx* factor = new ProjectionTwoFrameOneCamFactorNoEx(pts_i, pts_j, tic[0], Eigen::Quaterniond(ric[0]));
                        my_problem.AddResidualBlock(factor, loss_function,  para_pose[i2f[it_per_id.start_frame]], para_pose[i2f[imu_j]], &it_per_id.idepth_);
                    }
                ...
                if (USE_STEREO && it_per_frame.is_stereo) {
                    ...
                    if (it_per_id.start_frame != imu_j) {
                        ...
                        if (ESTIMATE_EXTRINSIC) {
                            ...
                            ProjectionTwoFrameTwoCamFactorNoEx* factor = new ProjectionTwoFrameTwoCamFactorNoEx(pts_i, pts_j_right, tic[0], Eigen::Quaterniond(ric[0]), tic[1], Eigen::Quaterniond(ric[1]));
                            my_problem.AddResidualBlock(factor, loss_function,  para_pose[i2f[it_per_id.start_frame]], para_pose[i2f[imu_j]], &it_per_id.idepth_);
                        }
                        ...
                        if (ESTIMATE_EXTRINSIC) {
                            ...
                            ceres::LossFunction* loss_function = new ceres::CauchyLoss(1.0);
                            ProjectionOneFrameTwoCamFactorNoEx* factor = new ProjectionOneFrameTwoCamFactorNoEx(pts_i, pts_j_right, tic[0], Eigen::Quaterniond(ric[0]), tic[1], Eigen::Quaterniond(ric[1]));
                            my_problem.AddResidualBlock(factor, loss_function,   &it_per_id.idepth_);
                        }
                ...
                if (ESTIMATE_EXTRINSIC) {
                    ...
                    ceres::LossFunction* loss_function = new ceres::CauchyLoss(1.0);
                    projection_factorNoEx1* factor = new projection_factorNoEx1(it_per_frame.point);
                    my_problem.AddResidualBlock(factor, loss_function, para_pose[i2f[imu_j]],  it_per_id.ptsInWorld.data());
                    if (USE_STEREO && it_per_frame.is_stereo) {
                        ceres::LossFunction* loss_function = new ceres::CauchyLoss(1.0);
                        projection_factorNoEx2* factor = new projection_factorNoEx2(it_per_frame.pointRight);
                        my_problem.AddResidualBlock(factor, loss_function, para_pose[i2f[imu_j]], it_per_id.ptsInWorld.data());
                    }
                }
    ...
    for (int i = 0; i < NUM_OF_CAM; i++) {
        if ((ESTIMATE_EXTRINSIC && Vs[g2f[rover_count - 1]].norm() > 0.2) || open_ex_estimation) {
            ...
            if (my_problem.HasParameterBlock(para_ex_Pose[i])) {
                ...
                constnum++;
                std::cout << "my_problem.HasParameterBlock(para_ex_Pose[i])" << std::endl;
            }
    ...
    if (USE_MAG_CORRECT_YAW) {
        if (frame_types[rover_count + image_count - 1] == ImagFrame) {
            if (my_problem.HasParameterBlock(para_bmg)) {
                ...
                constnum++;
                std::cout << "my_problem.HasParameterBlock(para_bmg[i])" << std::endl;
            }
        } else {
            if (my_problem.HasParameterBlock(para_bmg))
...
void SWFOptimization::GlobalMarge(std::set<double*>MargePoints) {
    ...
    for (int i = 0; i < (int)residual_blocks_all.size(); i++) {
        ASSERT(residual_blocks_all[i]->is_use);
    ...
    if (ceres::internal::parameter_head.size() == 0) {
        ...
        MyOrdering(my_problem, my_options);
        ceres::Solve(my_options, &my_problem, &summary);
        std::cout << "solving time global marg:" << t_marg.toc() << std::endl;
