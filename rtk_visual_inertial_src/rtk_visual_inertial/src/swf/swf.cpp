...
void SWFOptimization::SlideWindowFrame(int frameindex, int windowsize, bool updateIMU) {
    ...
    for (int i = frameindex; i < windowsize - 1; i++) {
        ...
        Rs[i] = Rs[i + 1];
        if (USE_MAG_CORRECT_YAW)mags[i] = mags[i + 1];
        Ps[i] = Ps[i + 1];
        Vs[i] = Vs[i + 1];
        Bas[i] = Bas[i + 1];
        Bgs[i] = Bgs[i + 1];
...
void SWFOptimization::MargFrames(std::set<int> margeindex) {
    ...
    if (USE_GLOBAL_OPTIMIZATION) {
        if (marg_flag == MargImagOld) {
            for (auto& it_per_id : f_manager.feature) {
                if (margeindex.find(i2f[it_per_id.start_frame]) != margeindex.end()) {
                    ...
                    if (my_problem.HasParameterBlock(&it_per_id.idepth_)) {
                        ...
                        ASSERT(it_per_id.feature_per_frame.size() >= FEATURE_CONTINUE);
            ...
            if (!USE_SPP_CORRECTION)
                ASSERT(MargePoints2.size() == 2);


            if (my_problem.HasParameterBlock(&blackvalue)) {
                std::vector<ceres::internal::ResidualBlock*>residual_blocks;
                my_problem.GetResidualBlocksForParameterBlock(&blackvalue, &residual_blocks);
                if (residual_blocks.size() == 1 && std::count(
                            std::begin(last_marg_info->keep_block_addr),
                            std::end(last_marg_info->keep_block_addr),
                            &blackvalue)
                   ) {
                    MargePoints2.insert(&blackvalue);
                    MargePoints.insert(&blackvalue);
                    // ASSERT(0);
                }
            }
            for (int i = 0; i < (int)last_marg_info->keep_block_addr.size(); i++) {
                double* addr = last_marg_info->keep_block_addr[i];
                if (MargePoints2.find(addr) != MargePoints2.end()) {
        ...
        if (marg_flag == MargImagOld) {
            for (auto& it_per_id : f_manager.feature) {
                if (it_per_id.feature_per_frame.size() < FEATURE_CONTINUE || margeindex.find(i2f[it_per_id.start_frame]) == margeindex.end())
                    continue;
                ASSERT(it_per_id.start_frame == 0);
#if USE_INVERSE_DEPTH
                MargePoints2.insert(&it_per_id.idepth_);
    ...
    if (marg_flag == MargImagOld || marg_flag == MargRoverOld) {
...
#if USE_INVERSE_DEPTH
    for (auto& it_per_id : f_manager.feature) {
        if ( it_per_id.feature_per_frame.size() >= FEATURE_CONTINUE  && (
                    (margeindex.find(i2f[it_per_id.start_frame]) != margeindex.end()) ||
                    (margeindex.find(i2f[it_per_id.endFrame()]) != margeindex.end() && it_per_id.feature_per_frame.size() == 2))  ) {
            if (my_problem.HasParameterBlock(&it_per_id.idepth_)) {
...
#define condition3 (USE_IMAGE&&!USE_GNSS&&!feature_buf.empty())
    ...
    while (condition1 || condition2 || condition3) {
        ...
        if (ENABLE_ONLINE &&  feature_buf.size() > 10) {
            static int count = 0;
            if (feature_buf.size()&&count++ % 5 == 0) {
                mutex_feature.lock();
                feature_buf.pop();
                mutex_feature.unlock();
                std::cout << "\033[32m" << count/5 << "," <<" pop IMAGE for realtime " << "buf size:" << rover_buf.size() << "," << feature_buf.size() << "\033[0m" << endl ;
                continue;
            }
        }
        TicToc t_process;
        ...
        if (modeflag == ImagFrame) {
            ...
            t_process2 += ts;
            t_count += 1;
            printf("process measurement time1: %f   ,%f   ,%d\n", ts, t_process2 / t_count, (int)modeflag);
        }
        ...
        if (modeflag == ImagFrame) {
            ...
            t_process2 += ts;
            t_count += 1;
            printf("process measurement time2: %f   ,%f   ,%d\n", ts, t_process2 / t_count, (int)modeflag);
        }
        ...
        if (modeflag == ImagFrame) {
            ...
            t_process2 += ts;
            t_count += 1;
            printf("process measurement time3: %f   ,%f   ,%d\n", ts, t_process2 / t_count, (int)modeflag);
        }
        if (last_marg_info) {
            for (int i = 0; i < (int)last_marg_info->keep_block_addr.size(); i++) {
                ASSERT(last_marg_info->keep_block_addr[i] != para_ex_Pose[0]);
                ASSERT(last_marg_info->keep_block_addr[i] != para_ex_Pose[1]);
                ASSERT(last_marg_info->keep_block_addr[i] != para_bmg);
                // ASSERT(last_marg_info->keep_block_addr[i]!=&blackvalue);
        ...
        if (modeflag == GnssFrame) {
            ...
            t_process2 += ts;
            t_count += 1;
            printf("process measurement time5: %f   ,%f   ,%d\n", ts, t_process2 / t_count, (int)modeflag);
        }
