...
class SWFOptimization {
  public:
    ...
    // mutex lock for imu measurements
    std::mutex mutex_imu;
    // mutex lock for mag measurements
    std::mutex mutex_mag;
    // mutex lock for gnss measurements
    std::mutex mutex_gnss;
    // mutex lock for feature measurements
    std::mutex mutex_feature;
    // mutex lock for output states
    std::mutex mutex_swf_states;
    //output states
    std::queue<Eigen::Matrix<double, 10, 1, Eigen::ColMajor>> swf_states;
