...
void SWFOptimization::UpdateSchur(ceres::Problem& problem) {
    ...
    if (m) {
        ...
        if (USE_LDLT_FOR_PSEUDO_INVERSE) {
            ...
            Amm_inv = saes2.eigenvectors() * Eigen::VectorXd((saes2.eigenvalues().array() > 1e-8).select(saes2.eigenvalues().array().inverse(), 0)).asDiagonal() * saes2.eigenvectors().transpose();
        }
...
void SWFOptimization::IntegerSolve() {
    ...
    t_sum += tic.toc();
    count++;
    std::cout << "LAMBDASearch:" << tic.toc() << "," << t_sum / count << std::endl;
...
#define CONDITION problem.HasParameterBlock(pointer)&&!problem.IsParameterBlockConstant(pointer)&&mark_add.find(pointer)==mark_add.end()
    ...
    if (need_black) {
        if (!problem.HasParameterBlock(&blackvalue2)) {
        ...
        if (problem.HasParameterBlock(&blackvalue2))
            problem.RemoveParameterBlock(&blackvalue2);
    }
    ...
    for (int i = 0; i < (int)last_marg_info->keep_block_size.size(); i++) {
        if (last_marg_info->keep_block_size[i] == 9)count++;
    }
    ASSERT(count <= 1);
#endif
