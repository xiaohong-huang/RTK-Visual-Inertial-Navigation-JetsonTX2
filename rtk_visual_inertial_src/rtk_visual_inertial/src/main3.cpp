...
void wait_for_time_reach(double time) {
    if (ENABLE_ONLINE) {
        static int count = 0;
        if (count++ < 100)return;
        static double time_shift = system_time.toc_s() - time;
        std::cout << "time gap:" << system_time.toc_s() << ","
                  << time << ","
                  << system_time.toc_s() - time_shift - time
                  << std::endl;
        while (system_time.toc_s() - time_shift <= time);

        if (system_time.toc_s() - time_shift - time > 1) {
            std::cout << "warning!! difficult for realtime" << std::endl;
        }
    }
...
void img0_callback(const sensor_msgs::ImageConstPtr& img_msg) {
    ...
    wait_for_time_reach(img_msg->header.stamp.toSec());
...
void feature_callback(const sensor_msgs::PointCloudConstPtr& feature_msg) {
    ...
    if (USE_IMAGE) {
        ...
        swf_optimization->mutex_feature.lock();
        swf_optimization->feature_buf.push(make_pair(timestamp, featureFrame));
        swf_optimization->Save();
        swf_optimization->mutex_feature.unlock();

        if (!USE_MULTI_THREAD)swf_optimization->MeasurementProcess();
    }
}
...
void bind_cpu(std::vector<int>cpu_set) {
    cpu_set_t mask;
    CPU_ZERO(&mask);
    for (int i = 0; i < (int)cpu_set.size(); i++) {
        CPU_SET(cpu_set[i], &mask);
    }
...
void optimization_process() {

    bind_cpu(std::vector<int>(SWF_CPU_SET));
    while (1) {
...
#if USE_MULTI_THREAD
    bind_cpu(std::vector<int>(MAIN_CPU_SET));
#else
    bind_cpu(std::vector<int>(SWF_CPU_SET));
#endif
...
#if USE_MULTI_THREAD
    std::thread optimization_thread {optimization_process};
#endif
