...
std::vector<Eigen::Quaterniond> QCI;
std::vector<Eigen::Vector3d> Pcg;
...
void readParameters(std::string config_file) {
    ...
    for (int i = 0; i < (int)RIC.size(); i++) {
        RCI.push_back(RIC[i].transpose());
        QCI.push_back(Eigen::Quaterniond(RCI[i]));
        Pcg.push_back(Pbg - TIC[i]);
    }
