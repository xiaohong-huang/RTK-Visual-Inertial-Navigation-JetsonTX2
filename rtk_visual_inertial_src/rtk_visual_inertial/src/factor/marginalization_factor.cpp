...
void MarginalizationInfo::marginalize_exclude() {
    ...
    if (USE_LDLT_FOR_PSEUDO_INVERSE) {
        ...
        for (int i = 0; i < diag.size(); i++)diag(i) = EPSS;
        Amm.diagonal() += diag;
        ...
        Amm_inv = saes2.eigenvectors() * Eigen::VectorXd((saes2.eigenvalues().array() > EPSS).select(saes2.eigenvalues().array().inverse(), 0)).asDiagonal() * saes2.eigenvectors().transpose();
    }
...
void MarginalizationInfo::marginalize(bool initialinformation, bool multithread) {
    ...
    if (m != 0) {
        ...
        if (USE_LDLT_FOR_PSEUDO_INVERSE) {
            ...
            Amm_inv = saes.eigenvectors() * Eigen::VectorXd((saes.eigenvalues().array() > EPSS).select(saes.eigenvalues().array().inverse(), 0)).asDiagonal() * saes.eigenvectors().transpose();
        }
    ...
    if (USE_LDLT_FOR_PSEUDO_INVERSE) {
        Eigen::VectorXd diag = Eigen::VectorXd(A.cols());
        for (int i = 0; i < diag.size(); i++)diag(i) = EPSS;
        A.diagonal() += diag;
        Eigen::LDLT<Eigen::MatrixXd, Eigen::Upper>ldlt = A.selfadjointView<Eigen::Upper>().ldlt();
        Eigen::VectorXd D = ldlt.vectorD();
        Eigen::Index a1, a2;
        if (ldlt.vectorD().minCoeff(&a1, &a2) > -0.1) {
            is_sucess = true;
            // ASSERT(ldlt.vectorD().minCoeff(&a1,&a2)>-0.1);
            D = ((D.array() > EPSS).select(D.array(), 0));

            Eigen::MatrixXd L = Eigen::MatrixXd::Identity(A.cols(), A.cols())
                                * ldlt.transpositionsP()
                                * ldlt.matrixL()
                                * D.cwiseSqrt().asDiagonal();

            linearized_jacobians = L.transpose();
            Eigen::VectorXd inc = ldlt.solve(b);
            linearized_residuals = linearized_jacobians * inc;
        }


    }
    if (!is_sucess || !USE_LDLT_FOR_PSEUDO_INVERSE) {
