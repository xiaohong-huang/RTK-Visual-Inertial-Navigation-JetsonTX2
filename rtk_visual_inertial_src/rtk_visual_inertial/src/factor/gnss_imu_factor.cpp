...
const int mapindex[O_Size - 1] = {O_N, O_Pose0, O_Pose1};
...
void IMUGNSSBase::SetLastImuFactor(IntegrationBase* pre_integration, double* para_Pose2, double* para_SpeedBias2) {
    ...
    param.push_back(para_pose0);
    param.push_back(para_speed_bias0);
    param.push_back(para_Pose2);
    param.push_back(para_SpeedBias2);
...
void IMUGNSSBase::UpdateSchurComponent() {
    ...
    if (USE_LDLT_FOR_PSEUDO_INVERSE) {
        ...
        // Eigen::LDLT<MatrixXd_,Eigen::Upper>ldlt=hessian_dense.selfadjointView<Eigen::Upper>().ldlt();
        // VectorXd_ D=ldlt.vectorD();
        // int index=0;
        // D=((D.array() > EPSS).select(D.array(), 0));
        // MatrixXd_ L=MatrixXd_::Identity(hessian_dense.cols(),hessian_dense.cols())
        //                                                 *ldlt.transpositionsP()
        //                                                 *ldlt.matrixL()
        //                                                 *D.cwiseSqrt().asDiagonal();

        // schur_jacobian=L.transpose();
        // VectorXd_ inc=ldlt.solve(rhs_dense);
        // schur_residual=schur_jacobian*inc;


        int size1 = hessian_size[O_N] + 15;
        int size2 = 15;
        for (int i = 0; i < hessian_size[O_N]; i++) {
            hessian_dense(i, i) += EPSS;
        }


        hessian_dense = hessian_dense.selfadjointView<Eigen::Upper>();
        Eigen::LLT<MatrixXd_, Eigen::Upper>llt = hessian_dense.block(0, 0,
                size1, size1).selfadjointView<Eigen::Upper>().llt();
        MatrixXd_ L = MatrixXd_::Zero(size1 + size2, size1 + size2);
        L.block(0, 0, size1, size1) = llt.matrixL();
        MatrixXd_ L_inv = MatrixXd_::Identity(size1, size1);
        llt.matrixL().solveInPlace(L_inv);
        L.block(size1, 0, size2, size1) = hessian_dense.block(size1, 0, size2, size1) * L_inv.transpose();
        L.block(size1, size1, size2, size2) =
            hessian_dense.block(size1, size1, size2, size2) -
            L.block(size1, 0, size2, size1) * L.block(size1, 0, size2, size1).transpose();

        Eigen::SelfAdjointEigenSolver<MatrixXd_> saes2(L.block(size1, size1, size2, size2));
        VectorXd_ S = VectorXd_((saes2.eigenvalues().array() > EPSS).select(saes2.eigenvalues().array(), 0));
        VectorXd_ S_sqrt = S.cwiseSqrt();
        L.block(size1, size1, size2, size2) = saes2.eigenvectors() * S_sqrt.asDiagonal();


        VectorXd_ S_inv = VectorXd_((saes2.eigenvalues().array() > EPSS).select(saes2.eigenvalues().array().inverse(), 0));
        VectorXd_ inc(size1 + size2);


        inc.segment(size1, size2) = saes2.eigenvectors() * S_inv.asDiagonal() * saes2.eigenvectors().transpose() *
                                    (rhs_dense.segment(size1, size2) - L.block(size1, 0, size2, size1) * L_inv * rhs_dense.segment(0, size1));

        VectorXd_ rhs2 = rhs_dense.segment(0, size1) - hessian_dense.block(0, size1, size1, size2) * inc.segment(size1, size2);
        inc.segment(0, size1) = llt.solve(rhs2);



        schur_jacobian = L.transpose();
        schur_residual = schur_jacobian * inc;
        // std::cout<<"cc:"<<(schur_jacobian.transpose()*schur_jacobian-hessian_dense).norm()<<","
        //     <<(schur_jacobian.transpose()*schur_residual-rhs_dense).norm()<<std::endl<<std::endl;
        // ASSERT(0);
...
void IMUGNSSBase::UpdateJacobResidual(double** jacobians, double* residuals) {
    ...
    if (jacobians) {
        ...
        for (int i = 0; i < hessian_size[O_N]; i++) {
            if (jacobians[ i]) {
                Eigen::Map<MatrixXd_>(jacobians[ i], size, 1) = schur_jacobian.block(0, hessian_index[0] + i, size, 1);
            }
        }

        if (jacobians[hessian_size[O_N] + 0]) {
            Eigen::Map<MatrixXd_>tmp(jacobians[hessian_size[O_N] + 0], size, 7);
            tmp.setZero();
            tmp.block(0, 0, size, 6) = schur_jacobian.block(0, hessian_index[1], size, 6);
        }
        if (jacobians[hessian_size[O_N] + 1]) {
            Eigen::Map<MatrixXd_>(jacobians[hessian_size[O_N] + 1], size, 9) = schur_jacobian.block(0, hessian_index[1] + 6, size, 9);
        }
        if (jacobians[hessian_size[O_N] + 2]) {
            Eigen::Map<MatrixXd_>tmp(jacobians[hessian_size[O_N] + 2], size, 7);
            tmp.setZero();
            tmp.block(0, 0, size, 6) = schur_jacobian.block(0, hessian_index[2], size, 6);
        }
        if (jacobians[hessian_size[O_N] + 3]) {
            Eigen::Map<MatrixXd_>(jacobians[hessian_size[O_N] + 3], size, 9) = schur_jacobian.block(0, hessian_index[2] + 6, size, 9);
        }
...
void IMUGNSSBase::UpdateDeltaValues(Vector7d_ &Pi, Vector9d_ &Bi, Vector7d_ &Pj, Vector9d_ &Bj, VectorXd_ &phase_biases_values) {
    ...
    int delta_count = 0;
    {
        ...
        INC.segment(hessian_size[O_N] + 15, 15) = dx;
        delta_sum += dx.norm();
        delta_count += 15;
    }

    {
        ...
        INC.segment(0, hessian_size[O_N]) = dx;
        delta_sum += dx.norm();
...

    {
        ...
        INC.segment(hessian_size[O_N], 15) = dx;
        delta_sum += dx.norm();
        delta_count += 15;
    }
    // if(sqrt(delta_sum)/delta_count>1e-3&&update_flag)update_flag=true;
...
bool IMUGNSSBase::Evaluate(double const* const* parameters, double* residuals, double** jacobians) {
    if (parameters == 0) {
        residuals[0] = gnss_speed_bias.size();
        return false;
    };
    Matrix15d_ J1;
    Matrix15d_ J2;
    ...
    double residual[15];
    evaluate_round++;
    for (int i = 0; i < hessian_size[O_N]; i++)phase_biases_values(i) = parameters[ i][0];
    for (int i = 0; i < 7; i++) {
        Pi(i) = parameters[ hessian_size[O_N] + 0][i];
        Pj(i) = parameters[hessian_size[O_N] + 2][i];
    }
    for (int i = 0; i < 9; i++) {
        Bi(i) = parameters[hessian_size[O_N] + 1][i];
        Bj(i) = parameters[hessian_size[O_N] + 3][i];
    }
    ...
    if (!history_flag || update_flag) {
        ...
        const double* parameter[4] = {parameters[hessian_size[O_N] + 0], parameters[hessian_size[O_N] + 1], gnss_poses[0], gnss_speed_bias[0]};
        ...
        for (int i = 0; i < (int)gnss_speed_bias.size(); i++) {
            ...
            if (i != (int)gnss_speed_bias.size() - 1) {
                ...
                parameter[0] = gnss_poses[i];
                parameter[1] = gnss_speed_bias[i];
                parameter[2] = parameters[hessian_size[O_N] + 2];
                parameter[3] = parameters[hessian_size[O_N] + 3];
